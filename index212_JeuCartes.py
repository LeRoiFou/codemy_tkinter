"""
Tkinter - Codemy.com #212 : Blackjack Convert Ace To One
...
Dans ce cours lorsque la 1ère posée est un As, cela vaut 1 point au lieu de 11 points
MAIS que la carte de l'As soit la 1ère carte posée ou la suivante, le score est toujours de 1 point...
    ...
    def blackjack_shuffle(self, gamer):
       ...
       # Total du score attribué à chaque participant
        ...
        if gamer == "dealer":
            ...
            if len(self.dealer_score) == 2:
                ...
                if self.dealer_score[0] + self.dealer_score[1] == 21 :
                    # Mise à jour du statut du 'dealer' : yes = gagné
        ...
        if gamer == "player":
            ...
            if len(self.player_score) == 2:
                ...
                if self.player_score[0] + self.player_score[1] == 21 :
                    # Mise à jour du statut du 'player' : yes = gagné
                ...
                for score in self.player_score:
                    ...
                    # Si le score total est de 21 points
                    ...
                    # Si le score total est > à 21 points
                        ...
                        for card_num, card in enumerate(self.player_score):
                            ...
                            if card == 11:
                                ...
                                # Réinitialisation du score total du player
                                self.player_total = 0
                                #
                                for score in self.player_score:
                                    self.player_total += score
        ...
        if len(self.dealer_score) == 2 and len(self.player_score) == 2:
            ...
            # Si le score des 2 cartes du 'dealer' est de 21 points
            ...
            # Si le score des 2 cartes du 'player' est de 21 points
        ...
        # Sinon pour toutes nouvelles cartes posées (> 2 cartes)        
            ...
            # Si le score des cartes du 'dealer' est de 21 points
            elif self.blackjack_status['dealer'] == 'yes':
                # Message informant que le dealer a gagné
                messagebox.showinfo("Dealer wins!", 
                                    "BlackJack! 21! Dealer wins!")
                # Désactivation des boutons
                self.card_button.config(state=DISABLED)
                self.stand_button.config(state=DISABLED)
                
            # Si le score des cartes du 'player' est de 21 points
            elif self.blackjack_status['player'] == 'yes':
                # Message informant que le dealer a gagné
                messagebox.showinfo("Player wins!", 
                                    "BlackJack! 21 points! Player wins!")
                # Désactivation des boutons
                self.card_button.config(state=DISABLED)
                self.stand_button.config(state=DISABLED)
        ...
        if self.blackjack_status['player'] == 'bust':
            # Message informant que le dealer a gagné
                messagebox.showinfo("Player busts!", 
                                    f"Player loses! {self.player_total} points!")
                # Désactivation des boutons
                self.card_button.config(state=DISABLED)
                self.stand_button.config(state=DISABLED)
