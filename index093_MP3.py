"""
Tkinter - Codemy.com #93 : Finishing Our Song Position Slider (MP3 Player | Part 7)
Lien : https://www.youtube.com/watch?v=gC8dRDqWw6I&list=PLCC34OHNcOtoC6GglhF3ncJ5rLwQrLGnV&index=93

Création d'un lecteur mp3 virtuel

Dans ce programme :
-> on supprime le temps affiché sous le curseur et on réduit donc la taille de l'écran
-> le curseur arrête d'avancer lorsque la musique est en mode 'pause dans la fonction play_time()'
-> on réinitialise la position du curseur lorsqu'on appuye sur le bouton 'stop dans la fonction stop()'
-> on réinitialise la position du curseur lorsqu'on supprime un titre dans la fonction delete_song() et lorsqu'on
supprime tous les titres de la playlist dans la fonction delete_all_songs() en appelant la fonction stop()
-> on réinitialise la position du curseur lorsqu'on passe au titre suivant dans la fonction next_song()
-> on réinitialise la position du curseur lorqu'on passe au titre précédent dans la fonction previous_song()
-> on réinitialise la position du curseur lorsqu'on appuye sur le bouton play() dans la fonction play()
