...
Dans ce cours on sauvegarde les couleurs paramétrées.
Pour cela on créé un fichier .ini en argumentant les couleurs
souhaitées avec :
-> L'importation d'un nouveau module : configparser
-> Création d'une nouvelle méthode : self.my_file_ini()
-> Modification des noms des couleurs définies dans 
la méthode self.widgets_treeview()
-> Modification dans les méthodes : self.primary_color(), 
self.secondary_color() et self.highlight_color()
-> Réinitialisation des couleurs d'origine en rajoutant
un nouvelle ligne du menu 'Options' et d'une nouvelle
méthode : reset_colors()
...
import sqlite3
# Import du fichier configurer sous l'extension .ini
...
class GUI:
    ...
    def my_file_ini(self):
        "Lecture du fichier configuré sous l'extension .ini"
        
        # Récupération du fichier .ini
        parser = ConfigParser()
        parser.read("database/tree_crm.ini")
        
        """ Récupération des données du fichier .ini
        Arguments de la fonction native get() :
        -> 1er argument : nom de la section dans le fichier .ini : [colors]
        -> 2ème argument : nom de la ligne dans le fichier .ini"""
        self.saved_primary_color = parser.get('colors', 'primary_color')
        self.saved_secondary_color = parser.get('colors', 'secondary_color')
        self.saved_highlight_color = parser.get('colors', 'highlight_color')
    ...
    def menu(self):
       ...
       option_menu.add_command(
           label="Réinitialiser les couleurs d'origine", 
    ...
    def widgets_treeview(self):
        ...
        Anciennement en deuxième argument : '#347083'
        Dorénavant en deuxième argument : 
        récupération des données du fichier .ini"""
        ...
        """Lignes impaires
        Anciennement : background='white'
        Dorénavant : récupération des données du fichier .ini"""
        self.my_tree.tag_configure(
            "oddrow", 
            background=self.saved_secondary_color) 
        
        """Lignes paires
        Anciennement : background='lightblue'
        Dorénavant : récupération des données du fichier .ini"""
        self.my_tree.tag_configure(
            "evenrow",
            background=self.saved_primary_color) 
    ...
    def primary_color(self):
        ...
        # Ouverture de la fenêtre du choix des couleurs
        ...
        if primary_color:
            ...
            # Récupération des données dans le fichier .ini
            parser = ConfigParser()
            parser.read("database/tree_crm.ini")
            
            # Modification de la nouvelle couleur choisie
            parser.set('colors', 'primary_color', primary_color)
            
            # Sauvegarde de la modification de couleur
            with open('database/tree_crm.ini', 'w') as configfile:
    ...
    def reset_colors(self):
        "Réinitialisation des couleurs d'origine"
        
        # Récupération des données dans le fichier .ini
        parser = ConfigParser()
        parser.read("database/tree_crm.ini")
         
        "Réitialisation des données dans le fichier .ini"
        # Lignes paires
        parser.set('colors', 'primary_color', 'lightblue')  
        # Lignes impaires
        parser.set('colors', 'secondary_color', 'white')
        # Couleur à la sélection
        ...
        "Réinitialisation des couleurs dans le widget treeview"
        # Lignes paires 
        self.my_tree.tag_configure(
            "evenrow",
