...
Dans ce programme on apprend à insérer un icône 
dans la barre d'outils de la fenêtre et à insérer une image
...
import tkinter
# Module pour importer des images dans tkinter : 
# package à télécharger -> Pillow
...
class GUI:
    ...
    def widgets(self):
        ...
        if now >= tictac:
            # L'image est insérée en tant qu'étiquette
            my_label = tkinter.Label(root, image=self.my_image)  
            my_label.pack()
        
        button_quite = tkinter.Button(
            root, 
