...
class GUI:
    ...
    def widgets_button(self):
        ...
        """Boutons"""
        move_up = tkinter.Button(root, text='Monter', command=self.up)
        move_up.pack(pady=10)

        move_down = tkinter.Button(root, text='Descendre', command=self.down)
        ...
        """Bindings"""
        self.my_tree.bind('<Double-1>', self.clicker)  # sélection d'une ligne saisie par un double clic gauche de la souris
        self.my_tree.bind('<ButtonRelease-1>', self.clicker)  # sélection d'une ligne saisie par un clic gauche de la souris
