"""
Tkinter - Codemy.com #91 : MP3 Player Song Slider (pt5)
Lien : https://www.youtube.com/watch?v=s_YUe0z09XU&list=PLCC34OHNcOtoC6GglhF3ncJ5rLwQrLGnV&index=91

Création d'un lecteur mp3 virtuel

Dans ce programme, on créé :
-> un curseur permettant de situer la musique en cours par rapport à la durée totale du titre
-> une étiquette affichant le temps par rapport à la position du curseur sur la durée totale du titre
...
def play():
    ...
    """Mise à jour de la position du curseur 
    Dans la création du curseur, on a mis comme argument to=100, mais pour que le curseur s'adapte à la durée totale du 
    titre, on remplace to=100 par to=int(song_lenght) qui est la durée totale du titre en secondes. 
    L'instruction value=0 permet de repositionner le curseur au début de la barre d'état à chaque fois qu'on appuye à  
    nouveau sur le bouton de lecture"""
    slider_position = int(song_length)  # variable sur la durée totale du titre en secondes (entiers)
    my_slider.config(to=slider_position, value=0)  # configuration de l'étiquette du curseur
