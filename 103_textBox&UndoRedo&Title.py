...
Dans ce programme on apprend à annuler et à rétablir une action.
Pour ces instructions, il n'y a pas lieu de crééer des fonctions, il suffit d'instruction l'instruction command dans le
widget qui effectuera l'action, et mettre l'instruction 'undo=True' dans le widget textbox

En complément on apprend à changer le titre de la fenêtre par le nom du fichier dans la fonction open_text()
...
def open_text():
    ...
    """Changement du titre de la fenêtre par le nom du fichier ouvert"""
    name = text_file
    name = name.replace('C:/Users/LRCOM/PycharmProjects/tests/pieces/', '')
...
"""Configuration du bouton d'exécution pour annuler une action"""
undo_button = Button(root, text='Annuler', command=my_text.edit_undo)
undo_button.pack(pady=5)

"""Configuration du bouton d'exécution pour rétablir une action"""
redo_button = Button(root, text='Rétablir', command=my_text.edit_redo)
