"""
Tkinter - Codemy.com #180 : Delete Many Selected Database Records - Python Tkinter GUI Tutorial 180
Lien : https://www.youtube.com/watch?v=LfzX6rBAGDs

Dans ce programme on apprend autre chose qu'à convertir une devise en une autre devise, 
pour cela on a recours à une API (interface de programmation applicative)...

On apprend à créer un CRM (Customer Relationship Management = Gestion de la relation client) 
avec le widget treeview (arborescence) et le recours à une base de données

Dans ce cours on enregistre dans le SGBD les multiples suppressions opérées en intervant dans la
méthode remove_many()
Un instruction supplémentaire a été ajoutée dans la partie langage SQL de cette méthode :
[(a,) for a in ids_to_delete]
Cette instruction permet d'arrêter l'erreur qui s'affiche, à savoir : 
"sqlite3.ProgrammingError: Incorrect number of bindings supplied. 
The current statement uses 1, and there are 2 supplied."
Car au-delà d'un composant d'un tuple supérieur à 9, au lieu de prendre :
(10) il prend (1,0) et au lieu de (11) il prend (1,1) et ainsi de suite...

À chaque fois qu'on intervient dans le SGBD, on a recours au langage SQL 
avec l'instruction c.execute()

https://youtu.be/tkBpehUruZ4?t=510
...
from tkinter import messagebox # pour le widget de message d'information/alerte...
import sqlite3
...
class GUI:
    ...
    def remove_all(self):
        ...
        # si suite au message d'information ci-dessus on clique sur "OUI"
        if response == 1:
            ...
            # suppression de toutes les données dans le SGBD (langage SQL)
    ...
    def remove_one(self):
        ...
        c.execute("DELETE from customers WHERE oid=" + self.id_entry.get())
    ...
    def remove_many(self):
        ...
        if response == 1:
            ...
            for record in x :
                ...
                ids_to_delete.append(self.my_tree.item(record, 'values')[2])     
