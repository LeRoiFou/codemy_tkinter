"""
Tkinter - Codemy.com #120 : Treeview Scrollbar - Python Tkinter GUI Tutorial #120
Lien : https://www.youtube.com/watch?v=-rVA37OVDs8&list=PLCC34OHNcOtoC6GglhF3ncJ5rLwQrLGnV&index=120

Dans ce programme on rattache une barre de défilement verticale à l'arborescence
...
class GUI:
    ...
    def widget_treeview(self):
        ...
        """Recours à une boucle for pour alimenter les données de la liste ci-dessus 
        dans l'arborescence et en attribuant une couleur selon si la ligne de rangée 
        ...
        for self.record in data:
            if self.count % 2 == 0:
                ...
                self.my_tree.insert(parent='', index='end', iid=self.count, text='', 
                    values=(self.record[0], self.record[1], self.record[2]), tags='evenrow')
            else:
                # Sinon la rangée est impaire et la couleur de la ligne est de couleur bleu claire
                self.my_tree.insert(parent='', index='end', iid=self.count, text='', 
                    values=(self.record[0], self.record[1], self.record[2]), tags='oddrow')
            self.count += 1
